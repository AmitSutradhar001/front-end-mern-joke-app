...
const JokesWithLimit = () => {
  ...
  const handleAPI = async () => {
    ...
    try {
      const res = await axios.get(
        `https://rest-api-jokes-app.onrender.com/api/${user.userKey}?limit=2`
      );
  ...
  return (
    <div className="sm:border-2 sm:ml-5 md:ml-10 border-black rounded-lg flex flex-col justify-start ">
      <h2 className="text-2xl font-bold px-5 pt-5">Jokes with limits</h2>
      <h2 className="text-lg font-bold px-5 pt-7">
        In this Jokes you will be able to set limits but limit should not be
        more than 50.
      </h2>
      <div className="flex flex-col md:flex-row md:gap-10">
        <h2 className="text-2xl font-bold px-5 pt-7">End Point:</h2>
        <h2 className="text-lg font-semibold px-5 pt-8">
          https://rest-api-jokes-app.onrender.com/api/key?limit=2
        </h2>
      </div>
      <div className="flex flex-col md:flex-row md:gap-10">
        <h2 className="text-2xl font-bold px-5 pt-7">Example:</h2>
        <h2 className="text-lg font-semibold px-5 pt-8">
          https://rest-api-jokes-app.onrender.com/api/asdfghjkl?limit=2
        </h2>
      </div>
      <div className="flex flex-col md:flex-row md:gap-10">
        <h2 className="text-2xl font-bold px-5 pt-7">Demo:</h2>
        <p className="text-lg font-semibold px-5 pt-8">
          axios.get(“https://rest-api-jokes-app.onrender.com/api/asdfghjkl?limit=2”)
          <br />
